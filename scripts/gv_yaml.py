...
def get_data(schema_path, data_path):
...
def make_observation_space(data):
    shape = Shape(*data['shape'])
    objects = list(map(make_object_type, data['objects']))
    colors = list(map(make_color, data['colors']))
...
def make_transition_function(data) -> transition_fs.StateDynamics:
...
def _make_transition_function(data) -> transition_fs.StateDynamics:
    return transition_fs.factory(data['name'])


def make_reward_function(data) -> reward_fs.RewardFunction:
    reward_functions = list(map(_make_reward_function, data))
    return partial(reward_fs.chain, reward_functions=reward_functions,)


def _make_reward_function(data) -> reward_fs.RewardFunction:
...
def make_terminating_function(data) -> terminating_fs.TerminatingFunction:
...
def main():
    ...
    parser.add_argument(
        '--schema', default='schema.yaml', help='Schema yaml file'
    )
    parser.add_argument('data', help='Data yaml file')
    args = parser.parse_args()

    data = get_data(args.schema, args.data)
    env = factory(data)
    print(env)
